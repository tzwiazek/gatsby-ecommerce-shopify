...
const Product = ({product}: {product: CartInterface}) => {
  ...
  if (colors.length === 0) {
    if (images[0].name.includes('gold') && !images[0].name.includes('rose-gold')) {
      ...
      currentColor = 'rose-gold'
    } else if (images[0].name.includes('silver')) {
      currentColor = 'silver'
    }
  } else {
    currentColor = colors[0].options[0].name;
  }
  ...
  function displayDefaultImage(): JSX.Element | JSX.Element[] {
    const frontImage: ImagesSharpInterface = findImage('front', product.images, currentColor);
    const backImage: ImagesSharpInterface = findImage('back', product.images, currentColor);
