...
const CartContext = React.createContext<CartContextType>([{}] as unknown as CartContextType);

const CartProvider = ({ children }: any) => {
  const [cart, setCart] = useState<CartInterface[]>([]);

  useEffect(() => {
    let storedCart: CartInterface[] = reactLocalStorage.getObject('cart') as CartInterface[];
  ...
  const removeItemFromCart = (id: string): void => {
    ...
    Object.assign(storedCart).splice(
      Object.assign(storedCart).findIndex((cartItem: CartInterface) => cartItem.id === id),
      1
    );
    setCart(storedCart);
  ...
  const changeItemQuantity = (id: string, num: number): void => {
    const storedCart = reactLocalStorage.getObject('cart') as CartInterface[];
    const currentItem = Object.assign(storedCart).findIndex(
      (cartItem: CartInterface) => cartItem.id === id
    );
  ...
  return (
    <CartContext.Provider
      value={[cart, setCart, emptyCart, removeItemFromCart, changeItemQuantity]}>
      {children}
    </CartContext.Provider>
  );
};
